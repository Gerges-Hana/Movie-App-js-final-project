  ...
  data.forEach((movie, index) => {
    ...
    movieEl.innerHTML = `
            ...
            <div class="overview">
                ...
                <button class="addToCart" id="${index}">Add To Cart <i class="fas fa-cart-arrow-down"></i></button>
            </div>
        `;
...
function addToCart(movie) {
  ...
  var receiveddata = JSON.stringify(movie);
  movies.push(movie);
...
function sortMovies() {
  ...
  // sort the array movies appling the rate
  var rate = [];
  var data = [];
  movies.forEach((movie, index) => {
    rate[index] = movie.vote_average
  })
  rate.sort(function (a, b) { return b - a });
  for (let i = 0; i < rate.length; i++) {
    for (let j = 0; j < movies.length; j++) {
     if(rate[i] == movies[j].vote_average){
      data[i]=movies[j]
      break;
    }
  }
...
function getMyCart(){
    if(localStorage.getItem('cart')){
        var array = localStorage.getItem('cart');
        var result=[];
        array = JSON.parse(array);
        array.forEach(element => {
            result.push(element)
        });
        showMoviesInsideTheGalary(result)
    }else{
...
function showMoviesInsideTheGalary(data) { 
  main.innerHTML = "";
  data.forEach((movie , index) => {
      const { title, poster_path, vote_average, overview, id } = movie;
    const movieEl = document.createElement("div");
    ...
    movieEl.innerHTML = `
            ...
            <div class="overview">
                ...
                <br/> 
                <button class="know-more" id="${id}">Watch Trailer <i class="fas fa-arrow-right"></i></button>                
                <button class="remove" id="${index}">Remove <i class="fas fa-cart-arrow-up"></i></button>
            </div>
        `;
        main.appendChild(movieEl);
      document.getElementById(id).addEventListener("click", () => {
...
function removeItem(index){
  ...
  showMoviesInsideTheGalary(array)
}
